...
std::shared_ptr<IClient> ServerImpl::client_factory(struct socket* s)
{
...
void ServerImpl::send(std::shared_ptr<IClient> c, const void* data, size_t len)
{
	c->send(data, len);
}

void ServerImpl::drop_client(std::shared_ptr<IClient> c)
{
	TRACE_func_entry(); BOOST_SCOPE_EXIT_ALL(&) { TRACE_func_left(); };
...
std::shared_ptr<IClient> ServerImpl::get_client(const struct socket* sock)
{
	std::lock_guard<std::mutex> _ {clients_mutex_};
