...
public:
   ...
   enum LogLevel {
	...
	using SCTPServer_cback_t = std::function<void(const std::shared_ptr<Client>, const std::string&)>;
	...
	*/
	...
	inline void send(std::shared_ptr<Client> c, const void* data, size_t len);

	/*
		Convenience wrapper
	*/
	inline void send(std::shared_ptr<Client> c, const std::string& message);
...
private:
	...
	void handle_client_data(std::shared_ptr<Client> c, const void* buffer, ssize_t n,
	...
	ssize_t send_raw(std::shared_ptr<Client>& c, const void* buf, size_t len);

	void drop_client(std::shared_ptr<Client>);
